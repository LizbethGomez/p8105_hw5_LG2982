...
output_10K %>% 
  ...
  summarise(est_mean =mean(estimate),
            est_var = var(estimate)) %>% 
  
knitr::kable(diggits = 3)
```

    ## Warning in mean.default(estimate): argument is not numeric or logical:
    ## returning NA
    
    ## Warning in mean.default(estimate): argument is not numeric or logical:
    ## returning NA

    ## Warning in var(estimate): NAs introduced by coercion
    
    ## Warning in var(estimate): NAs introduced by coercion

| estimate | est\_mean | est\_var |
| :------- | --------: | -------: |
| estimate |        NA |       NA |
| p.value  |        NA |       NA |
